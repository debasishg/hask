...
-- the `asOn` date
allTransactionsAsOn :: Text -> UTCTime -> IO [Transaction]
allTransactionsAsOn aNo asOn = 
  runSqliteAction $ 
...
-- `Monoid` instance
balanceAsOn :: Text -> UTCTime -> IO (Y.Dense "USD")
balanceAsOn aNo asOn = do
  txns <- allTransactionsAsOn aNo asOn
