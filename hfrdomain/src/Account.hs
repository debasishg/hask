...
makeAccount :: Value -> UTCTime -> Validation (NonEmpty ErrorInfo) Account
makeAccount req currentTime = case validationToEither $ _makeAccount req currentTime of
  Right a -> validate a
  Left  e -> eitherToValidation (Left e) 

_makeAccount :: Value -> UTCTime -> Validation (NonEmpty ErrorInfo) Account
_makeAccount req utcCurrent = 
...
validateOpenCloseDate acc = undefined 

validateRateOfInterest :: Account -> Validation (NonEmpty ErrorInfo) Account 
validateRateOfInterest acc = undefined 
...
-- validateAccountCloseDate v openDate = case validationToEither $ asDate v of
--   Left  e  -> eitherToValidation (Left e)
--   Right dt -> if dt < openDate
--                 then failure $ InvalidAccountOpenCloseDateCombination (T.pack $ "Account close date " ++ show dt ++ " cannot precede open date " ++ show openDate)
--                 else Success dt
...
parseRateOfInterest :: AccountType -> Value -> Validation (NonEmpty ErrorInfo) Double
parseRateOfInterest Ch v = case validationToEither $ asDouble v of
  Right i -> if i == 0.0 then Success i else failure RateNotApplicableForCheckingAccount
  Left  e -> eitherToValidation (Left e)
